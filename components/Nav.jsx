  ...
  return (
    <nav className="flex w-full h-20 border-[#E6E6E6] border-b-4 rounded-md flex-row items-center justify-between z-10 px-10">
      ...
      <div className="flex flex-col justify-around w-10 h-8 ml-10 z-40 relative" onClick={toggleMenu}>
        <div className={`flex w-full h-[3px] rounded-sm transition-all duration-[600ms] ease-in-out ${isOpen ? 'bg-[#EEEAE4] -translate-y-1' : 'bg-gray-800'}`}></div>
        <div className={`flex w-full h-[3px] rounded-sm transition-all duration-[600ms] ease-in-out ${isOpen ? 'opacity-0 translate-x-5 bg-[#EEEAE4]' : 'bg-gray-800'}`}></div>
        <div className={`flex w-full h-[3px] rounded-sm transition-all duration-[600ms] ease-in-out ${isOpen ? 'bg-[#EEEAE4] translate-y-1' : 'bg-gray-800'}`}></div>
      </div>
    </nav>
  );
